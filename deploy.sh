...
ensure_network() {
  if ! docker network inspect mean-net >/dev/null 2>&1; then
    docker network create mean-net
  fi
}

ensure_mongo() {
  docker compose -f "$APP_DIR/docker-compose.mongo.yml" up -d
}
...
docker compose -f "$APP_DIR/docker-compose.app.yml" -p "mean-${COLOR}" pull
FRONTEND_PORT="$FRONTEND_PORT" BACKEND_PORT="$BACKEND_PORT" \
  docker compose -f "$APP_DIR/docker-compose.app.yml" -p "mean-${COLOR}" up -d
